...
final class GlobalContentOperations
{
	...
	function LoadHtmlBlobs()
	{
		...
		$query = "SELECT htmlblob_id, htmlblob_name, html, htmlen, owner, modified_date, use_wysiwyg, description FROM ".cms_db_prefix()."htmlblobs ORDER BY htmlblob_name";
		...
		while (is_object($dbresult) && !$dbresult->EOF)
		{
			...
			$oneblob->contentEn = $dbresult->fields['htmlen'];
	...
	function &LoadHtmlBlobByID($id)
	{
		...
		$query = "SELECT htmlblob_id, htmlblob_name, html, htmlen, owner, modified_date, description, use_wysiwyg FROM ".cms_db_prefix()."htmlblobs WHERE htmlblob_id = ?";
	...
	function &LoadHtmlBlobByName($name)
	{
		...
		$query = "SELECT htmlblob_id, htmlblob_name, html, htmlen, owner, use_wysiwyg, description, modified_date FROM ".cms_db_prefix()."htmlblobs WHERE htmlblob_name = ?";
	...
	function InsertHtmlBlob($htmlblob)
	{
		...
		$query = "INSERT INTO ".cms_db_prefix()."htmlblobs (htmlblob_id, htmlblob_name, html, htmlen, owner, use_wysiwyg, description, create_date, modified_date) VALUES (?,?,?,?,?,?,?,".$time.",".$time.")";
		$dbresult = $db->Execute($query, array($new_htmlblob_id, $htmlblob->name, $htmlblob->content, $htmlblob->contentEn, $htmlblob->owner, $htmlblob->use_wysiwyg, $htmlblob->description));
		if ($dbresult !== false)
		{
	...
	function UpdateHtmlBlob($htmlblob)
	{
		...
		$time = $db->DBTimeStamp(time());
		$query = "UPDATE ".cms_db_prefix()."htmlblobs SET htmlblob_name = ?, html = ?, htmlen = ?, owner = ?, use_wysiwyg = ?, description = ?, modified_date = ".$time." WHERE htmlblob_id = ?";
		$dbresult = $db->Execute($query,array($htmlblob->name,$htmlblob->content,$htmlblob->contentEn,$htmlblob->owner,$htmlblob->use_wysiwyg,$htmlblob->description,$htmlblob->id));
		if ($dbresult !== false)
		{
			$result = true;
		}
