	...
	$(".inputs").each(function(i){//for each set of .inputs being set1, set2, set3 etc...
		...
		function repeat(currentSet){
			...
			$("#" + currentSet + " .formInputs:last .add:visible").click(function(){//add click handler to current sets .add ex. = #set1Input1 .add
				...
				$("#" + currentSet + " .formInputs:last").after("<span class='formInputs' id='"+currentSet+"Input"+($("#" + currentSet + " .formInputs").length + 1)+"'><label>name<input name='name' alt='name' type='text' /></label>&nbsp;<label>last name<input name='lastName' alt='lastName' type='text' /></label>&nbsp;<label>email<input name='email' alt='email' type='text' /></label>&nbsp;<a href='#' class='remove'><img src='removeBtn.gif' alt='remove' /></a>&nbsp;<a href='#' class='add'><img src='addBtn.gif' alt='add' /></a></span>")// add another set of inputs for a specific set
