...
//		$cislof = str_replace("[amp]","&",strip_tags($_GET["cislof"]));
		...
		//die($sql);
		if(($res = pg_query($sql)) == false)
			die("nepodarilo sa upraviť položku s id $id_item");
		else 
			die(show_items($key,"snimace"));		
	}
	...
	elseif($action == "update_sirena"){
		$key = strip_tags($_GET["key"]);
		$id_item = strip_tags($_GET["id_item"]);
		$id = strip_tags($_GET["id"]);
		$cislo = str_replace("[amp]","&",strip_tags($_GET["cislo"]));
		$umiestnenie = strip_tags($_GET["umiestnenie"]);
		$typ = strip_tags($_GET["typ"]);
		$zalohovana = strip_tags($_GET["zalohovana"]);
		$funkcnost = strip_tags($_GET["funkcnost"]);
		
		$sql = "UPDATE inspection_form_sireny SET id_sirena='$id', cislo='$cislo', umiestnenie='$umiestnenie', typ='$typ', zalohovana='$zalohovana', funkcnost='$funkcnost' WHERE id = $id_item AND key = $key";
		//die($sql);
		if(($res = pg_query($sql)) == false)
			die("nepodarilo sa upraviť položku s id $id_item");
		else 
