...
if (isset($_POST['contentEn'])) $contentEn = $_POST['contentEn'];
  ...
  if (isset($_POST["submit2"]) || isset($_POST['apply']) || $ajax)
	{
		...
		elseif(preg_match('<^[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*$>', $htmlblob) == 0) {
		  ...
		  $error .= '<li>'.lang('nofieldgiven',array('Obsah En')).'</li>';
		  $validinfo = false;
		}
...
<div class="pagecontainer">
	...
	<form id="Edit_Blob" method="post" action="edithtmlblob.php">
		...
		<div class="pageoverflow">
			...
			<p class="pageinput"><?php echo create_textarea($gcb_wysiwyg && $use_wysiwyg, $contentEn, 'contentEn', 'wysiwyg', 'contentEn');?></p>
		</div>
		<div class="pageoverflow"  style="margin-top:40px">
		       	<p class="pageinput">
		       		<input type="radio" name="description" <?php echo (($description == "1") ? "checked='checked'" : "")?> value="1">Zobrazovať<br>
					<input type="radio" name="description" <?php echo (($description == "0") ? "checked='checked'" : "")?> value="0">Nezobrazovať
	       		</p>
		</div>
               																		               
	<?php if ($adminaccess) { ?>
		<div class="pageoverflow" style="visibility:hidden">
